...
class PPO_tunado(PPO):
        def __init__(self, directory: str, policy: str, env: gymnasium.Env, ref_agent: Optional[str], ref_control: Optional[Controller], calc_mutual_info: bool, hparams: dict):            
                ...
                if self.calc_mutual_info:
                    with torch.no_grad():
                        ...
                        for key in metrics['actor']['gradient'].keys():
                            self.logger.record(f"train/actor_weight_layer_{key}", np.mean(metrics['actor']['weights'][key]))
                            self.logger.record(f"train/actor_grad_layer_{key}", np.mean(metrics['actor']['gradient'][key]))
                            self.logger.record(f"train/actor_mean_grad_layer_{key}", np.mean(metrics['actor']['grad_mean'][key]))
                            self.logger.record(f"train/actor_std_grad_layer_{key}", np.mean(metrics['actor']['grad_std'][key]))
                        
                        for key in metrics['critic']['gradient'].keys():
                            self.logger.record(f"train/critic_weight_layer_{key}", np.mean(metrics['critic']['weights'][key]))
                            self.logger.record(f"train/critic_grad_layer_{key}", np.mean(metrics['critic']['gradient'][key]))
                            self.logger.record(f"train/critic_mean_grad_layer_{key}", np.mean(metrics['critic']['grad_mean'][key]))
                            self.logger.record(f"train/critic_std_grad_layer_{key}", np.mean(metrics['critic']['grad_std'][key]))

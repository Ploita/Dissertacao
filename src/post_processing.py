...
def apply_latex_legend(col_name, type):
    ...
    if type == "loss":
        ...
        match = re.search(
            r"(actor|critic)[._\s]?(weight|grad)[._\s]?layer(?:[._\s](\d+))?.(weight|bias)",
            col_name,
        )
        if match:
...
def gera_combinacoes(colunas_mi_seguras: list) -> list[tuple[str, str]]:
    ...
    A entrada agora é uma lista de strings de chaves seguras
    (ex: ['actor_I_X_h1', 'actor_I_h1_h2', ...]).
...
def load_and_group_data(df):
    ...
    if "id_experimento" not in df.columns:
        print("Erro: Coluna 'id_experimento' não encontrada no DataFrame compilado.")
        return None
    ...
    metric_map = OrderedDict(
        [
            (
                "loss",
                {
                    "title": "Componentes da Loss",
                    "ylabel": "Valor da Loss Média",
                    "components": [
                        "entropy_loss",
            ...
            (
                "actor_grad",
                {
                    "title": "Gradiente do Ator",
                    "ylabel": "Valor do Gradiente",
                    "components": None,
                    "type": "grad",
                },
            ),
...
def plot_im(data: pd.DataFrame, directory: str):
    ...
    # gera_combinacoes extrai as chaves SEGURAS (ex: 'I_X_h1')
    # a partir das colunas de MI.
    # Ex: ['actor_I_X_h1_mean', 'actor_I_h1_h2_mean', ...]
    # retorna [('I_X_h1', 'I_h1_h2'), ...].

...
class MutualInfoCalculator:
    ...
    def compute(self, metrics: dict, fetcher: ActivationFetcher, observations: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            fetcher.activations["actor_hat Y"] = (
                self.model.policy.action_net(self.model.policy.mlp_extractor.policy_net(features))
                .cpu()
                .numpy()
            )
            fetcher.activations["critic_hat Y"] = (
                self.model.policy.value_net(self.model.policy.mlp_extractor.value_net(features))
                .cpu()
                .numpy()
            )
        ...
        for key_safe, (raw1, raw2) in self.mapping.items():
            metrics["actor"]["mutual_info"][key_safe].append(
                ee.mi(fetcher.activations[f"actor_{raw1}"], fetcher.activations[f"actor_{raw2}"])
            )
            metrics["critic"]["mutual_info"][key_safe].append(
                ee.mi(fetcher.activations[f"critic_{raw1}"], fetcher.activations[f"critic_{raw2}"])
            )

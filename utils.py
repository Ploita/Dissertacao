...
class PPO_tunado(PPO):
        ...
        def train(self):
            ...
            for epoch in range(self.n_epochs):
                ...
                mode = 'a' if os.path.exists('resultados.csv') else 'w'
                df.to_csv('resultados.csv', mode=mode, index=False, header=not os.path.exists('resultados.csv'))
                #end - CSV writing
...
class Experimento():
    ...
    def treinamento(self):
        ...
        self.model = PPO_tunado('MlpPolicy', self.train_env, policy_kwargs= dict(net_arch = dict(pi=self.size, vf=self.size)), device= self.device) 
        #* Não tenho interesse em deixar o ator/crítico com tamanho diferente
    ...
    def visualizar_modelo_ppo(self):
